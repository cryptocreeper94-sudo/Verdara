...
- **Data & Storage:** PostgreSQL database managed with Drizzle ORM, including seeding scripts and a living catalog of 177+ US outdoor locations (including 32 Tennessee locations across state parks, national park sites, private parks, botanical gardens, recreation areas, and scenic attractions — all with coffee-table-book quality descriptions, foraging guides, and species data). Tennessee catalog seed in `server/tennessee-catalog-seed.ts`. Location types: `state_park`, `national_park`, `private_park`, `recreation_area`, `botanical_garden`, `scenic_attraction`. Catalog content is designed to be print-publish ready for a future outdoor coffee table book.
- **Core Modules:**
    ...
    - **Hallmark System:** Trust Layer ecosystem audit trail (App #26, prefix `VD`). Genesis hallmark `VD-00000001` created on first boot with SHA-256 hashing, simulated blockchain fields (txHash, blockHeight), and parentGenesis reference to `TH-00000001`. Public verify endpoint at `/api/hallmark/:id/verify`. Trust stamps auto-generated for auth events (login, register, logout) with standardized categories. Server module: `server/hallmark.ts`.
    - **Affiliate Program:** Ecosystem-wide referral system using user `uniqueHash`. Five commission tiers (Base 10% → Diamond 20%). Referral links at `/ref/:hash`. Affiliate dashboard at `/affiliate` with tier progress, referral stats, commission tracking, and payout requests (min 10 SIG). Cross-platform referral links to other Trust Layer apps. Server module: `server/affiliate.ts`. Share & Earn card on Command Center, genesis badge on Profile.
