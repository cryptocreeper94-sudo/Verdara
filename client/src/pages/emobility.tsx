...
import { Zap, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, Battery, CircuitBoard } from "lucide-react";
...
export default function Emobility() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=emobility&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=emobility"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-emobility">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-emobility.jpg"
          alt="Electric Mobility"
          className="w-full h-full object-cover"
          data-testid="img-hero-emobility"
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Explore e-bike friendly trails, charging stations, and routes for Onewheels, EUCs, and electric scooters.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-[200px] flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              type="text"
              placeholder="Search e-mobility locations by name or location..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent outline-none text-sm text-foreground placeholder:text-muted-foreground"
              data-testid="input-emobility-search"
            />
            {searchQuery && (
          ...
          {filteredSpots.map((spot, i) => (
            ...
            >
              <Card
                ...
                data-testid={`card-emobility-${spot.id}`}
              >
                <div className="relative h-48 overflow-hidden rounded-t-md">
                  <img
                    src={spot.image || "/images/activity-ebike.jpg"}
                    alt={spot.name}
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {spot.metadata?.charging_stations && (
                        <Badge variant="outline" className="text-amber-400 border-amber-500/30 text-xs" data-testid={`badge-charging-${spot.id}`}>
                          <Battery className="w-3 h-3 mr-1" />
                          {spot.metadata.charging_stations} Charging
                        </Badge>
                      )}
                    </div>
                  </div>
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            <CircuitBoard className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No e-mobility locations found</h3>
