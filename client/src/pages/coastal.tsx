...
import { Shell, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, Waves, Droplets, Thermometer } from "lucide-react";
...
export default function Coastal() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=coastal&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=coastal"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-coastal">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-coastal.jpg"
          alt="Coastal & Beach"
          className="w-full h-full object-cover"
          data-testid="img-hero-coastal"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/20 to-background" />
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          <div className="flex items-center gap-3 mb-2">
            <Shell className="w-7 h-7 text-emerald-400" />
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Explore beaches, coastal trails, tide pools, surf spots, and seaside camping. Discover marine wildlife and coastal adventures.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-[200px] flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              type="text"
              placeholder="Search coastal and beach spots by name or location..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent outline-none text-sm text-foreground placeholder:text-muted-foreground"
              data-testid="input-coastal-search"
            />
            {searchQuery && (
        ...
        <p className="text-sm text-muted-foreground mb-5" data-testid="text-results-count">
          {filteredSpots.length} coastal spot{filteredSpots.length !== 1 ? "s" : ""} found
        </p>
            ...
            return (
              ...
              >
                <Card
                  ...
                  data-testid={`card-coastal-${spot.id}`}
                >
                  <div className="relative h-48 overflow-hidden rounded-t-md">
                    <img
                      src={spot.image || "/images/cat-coastal.jpg"}
                      alt={spot.name}
                    ...
                    {spot.species && spot.species.length > 0 && (
                      <div data-testid={`species-list-${spot.id}`}>
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <Shell className="w-3.5 h-3.5 text-emerald-400" />
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No coastal spots found</h3>
