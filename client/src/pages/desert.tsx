...
import { Sun, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, Mountain, Droplets } from "lucide-react";
...
export default function Desert() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=desert&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=desert"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-desert">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-desert.jpg"
          alt="Desert & Canyon"
          className="w-full h-full object-cover"
          data-testid="img-hero-desert"
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Discover desert trails, slot canyons, rock formations, and stargazing spots. Navigate arid landscapes with safety tips and water sources.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-[200px] flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              type="text"
              placeholder="Search desert and canyon areas by name or location..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent outline-none text-sm text-foreground placeholder:text-muted-foreground"
              data-testid="input-desert-search"
            />
            {searchQuery && (
        ...
        <p className="text-sm text-muted-foreground mb-5" data-testid="text-results-count">
          {filteredSpots.length} desert area{filteredSpots.length !== 1 ? "s" : ""} found
        </p>
            ...
            return (
              ...
              >
                <Card
                  ...
                  data-testid={`card-desert-${spot.id}`}
                >
                  <div className="relative h-48 overflow-hidden rounded-t-md">
                    <img
                      src={spot.image || "/images/cat-desert.jpg"}
                      alt={spot.name}
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No desert areas found</h3>
