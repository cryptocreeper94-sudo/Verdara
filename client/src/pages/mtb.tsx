...
import { Bike, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, Mountain } from "lucide-react";
...
export default function Mtb() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=mtb&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=mtb"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-mtb">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-mtb.jpg"
          alt="Mountain Biking"
          className="w-full h-full object-cover"
          data-testid="img-hero-mtb"
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Discover world-class MTB trails, bike parks, and singletrack across the country. Find your next ride with difficulty ratings and trail features.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
          ...
          {filteredSpots.map((spot, i) => (
            ...
            >
              <Card
                ...
                data-testid={`card-mtb-${spot.id}`}
              >
                <div className="relative h-48 overflow-hidden rounded-t-md">
                  <img
                    src={spot.image || "/images/activity-biking.jpg"}
                    alt={spot.name}
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {spot.metadata?.trail_count && (
                        <Badge variant="outline" className="text-amber-400 border-amber-500/30 text-xs" data-testid={`badge-trail-count-${spot.id}`}>
                          {spot.metadata.trail_count} Trails
                        </Badge>
                      )}
                      {spot.metadata?.lift_served === "true" && (
                        <Badge variant="outline" className="text-emerald-400 border-emerald-500/30 text-xs" data-testid={`badge-lift-served-${spot.id}`}>
                          Lift-Served
                        </Badge>
                      )}
                    </div>
                  ...
                  {spot.species && spot.species.length > 0 && (
                    <div data-testid={`species-list-${spot.id}`}>
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        <span className="text-xs font-medium text-muted-foreground">Trail Types</span>
                      </div>
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No trails found</h3>
