...
import { Fish, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, Anchor } from "lucide-react";
...
export default function Fishing() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=fishing&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=fishing"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-fishing">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-fishing.jpg"
          alt="Fishing"
          className="w-full h-full object-cover"
          data-testid="img-hero-fishing"
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Discover the best fishing locations across the country. Find your next catch with detailed spot info, species, and seasonal guides.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-[200px] flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              type="text"
              placeholder="Search fishing spots by name or location..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent outline-none text-sm text-foreground placeholder:text-muted-foreground"
              data-testid="input-fishing-search"
            />
            {searchQuery && (
        ...
        <p className="text-sm text-muted-foreground mb-5" data-testid="text-results-count">
          {filteredSpots.length} fishing spot{filteredSpots.length !== 1 ? "s" : ""} found
        </p>
          ...
          {filteredSpots.map((spot, i) => (
            ...
            >
              <Card
                ...
                data-testid={`card-fishing-${spot.id}`}
              >
                <div className="relative h-48 overflow-hidden rounded-t-md">
                  <img
                    src={spot.image || "/images/activity-fishing.jpg"}
                    alt={spot.name}
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No fishing spots found</h3>
