...
import { Anchor, Fish, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, Users } from "lucide-react";
...
export default function Charters() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=charters&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=charters"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-charters">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-fishing.jpg"
          alt="Fishing & Hunting Charters"
          className="w-full h-full object-cover"
          data-testid="img-hero-charters"
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Book deep sea fishing charters, guided hunts, and outdoor expeditions. Browse captain profiles, pricing, and availability.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-[200px] flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              ...
              data-testid="input-charters-search"
            />
            {searchQuery && (
        ...
        <p className="text-sm text-muted-foreground mb-5" data-testid="text-results-count">
          {filteredSpots.length} charter{filteredSpots.length !== 1 ? "s" : ""} found
        </p>
            ...
            return (
              ...
              >
                <Card
                  ...
                  data-testid={`card-charters-${spot.id}`}
                >
                  <div className="relative h-48 overflow-hidden rounded-t-md">
                    <img
                      src={spot.image || "/images/activity-charter.jpg"}
                      alt={spot.name}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {meta?.max_passengers && (
                        <Badge variant="outline" className="text-xs text-amber-400 border-amber-500/30">
                          ...
                          Up to {meta.max_passengers} Guests
                        </Badge>
                      )}
                      {meta?.charter_type && (
                    ...
                    {spot.species && spot.species.length > 0 && (
                      <div data-testid={`species-list-${spot.id}`}>
                        <div className="flex items-center gap-1.5 mb-1.5">
                          ...
                          <span className="text-xs font-medium text-muted-foreground">Target Species</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {spot.species.map(sp => (
                            <Badge
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No charters found</h3>
