...
import { Mountain, Search, MapPin, Star, Heart, X, Loader2, Shield, Compass, Tent } from "lucide-react";
...
function getDifficultyColor(difficulty: string | null) {
  if (!difficulty) return "bg-slate-500/15 text-slate-400";
  for (const key of Object.keys(difficultyColors)) {
    if (difficulty.toLowerCase().includes(key.toLowerCase())) return difficultyColors[key];
  }
...
export default function Climbing() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=climbing&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=climbing"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-climbing">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-climbing.jpg"
          alt="Rock Climbing"
          className="w-full h-full object-cover"
          data-testid="img-hero-climbing"
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Explore top-rated climbing areas across the country. Find crags, bouldering fields, and multi-pitch routes with detailed grades and local beta.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-[200px] flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              type="text"
              placeholder="Search climbing areas by name or location..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent outline-none text-sm text-foreground placeholder:text-muted-foreground"
              data-testid="input-climbing-search"
            />
            {searchQuery && (
        ...
        <p className="text-sm text-muted-foreground mb-5" data-testid="text-results-count">
          {filteredAreas.length} climbing area{filteredAreas.length !== 1 ? "s" : ""} found
        </p>
          ...
          {filteredAreas.map((area, i) => (
            ...
            >
              <Card
                ...
                data-testid={`card-climbing-${area.id}`}
              >
                <div className="relative h-48 overflow-hidden rounded-t-md">
                  <img
                    src={area.image || "/images/activity-climbing.jpg"}
                    alt={area.name}
                  ...
                  {area.difficulty && (
                    <div className="absolute top-3 left-3">
                      <Badge className={getDifficultyColor(area.difficulty)} data-testid={`badge-difficulty-${area.id}`}>
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  {area.tags && area.tags.length > 0 && (
                    <div data-testid={`tags-list-${area.id}`}>
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <Mountain className="w-3.5 h-3.5 text-emerald-400" />
                        <span className="text-xs font-medium text-muted-foreground">Climbing Style</span>
                      </div>
                  ...
                  {area.amenities && area.amenities.length > 0 && (
                    <div data-testid={`amenities-list-${area.id}`}>
                      <div className="flex items-center gap-1.5 mb-1.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {area.amenities.map(amenity => (
                          <Badge
                            ...
                            data-testid={`badge-amenity-${area.id}-${amenity.toLowerCase().replace(/\s+/g, "-")}`}
                          >
                            {amenity}
                          </Badge>
                        ))}
        ...
        {filteredAreas.length === 0 && !isLoading && (
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No climbing areas found</h3>
