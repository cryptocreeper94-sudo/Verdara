...
import { Tent, Search, MapPin, Star, Heart, X, ChevronDown, Loader2, Calendar, Shield, TreePine, CheckCircle } from "lucide-react";
...
export default function Camping() {
  ...
  const queryKey = searchQuery
    ? ["/api/activities", `?type=camping&q=${encodeURIComponent(searchQuery)}`]
    : ["/api/activities", "?type=camping"];
  ...
  return (
    <div className="min-h-screen" data-testid="page-camping">
      <div className="relative h-52 md:h-64 overflow-hidden">
        <img
          src="/images/cat-camping.jpg"
          alt="Camping"
          className="w-full h-full object-cover"
          data-testid="img-hero-camping"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/20 to-background" />
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          <div className="flex items-center gap-3 mb-2">
            <Tent className="w-7 h-7 text-emerald-400" />
          ...
          <p className="text-white/70 text-sm mt-1 drop-shadow max-w-lg" data-testid="text-page-description">
            Find the perfect campsite from developed campgrounds to remote dispersed spots. Filter by amenities, location, and season.
          </p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 py-6">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex-1 min-w-0 flex items-center gap-2.5 bg-card border border-card-border rounded-xl px-4 py-3">
            ...
            <input
              ...
              data-testid="input-camping-search"
            />
            {searchQuery && (
        ...
        <p className="text-sm text-muted-foreground mb-5" data-testid="text-results-count">
          {filteredSpots.length} campground{filteredSpots.length !== 1 ? "s" : ""} found
        </p>
          ...
          {filteredSpots.map((spot, i) => (
            ...
            >
              <Card
                ...
                data-testid={`card-camping-${spot.id}`}
              >
                <div className="relative h-48 overflow-hidden rounded-t-md">
                  <img
                    src={spot.image || "/images/activity-camping.jpg"}
                    alt={spot.name}
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {spot.metadata?.site_count && (
                        <Badge variant="outline" className="text-amber-400 border-amber-500/30 text-xs" data-testid={`badge-site-count-${spot.id}`}>
                          {spot.metadata.site_count} Sites
                        </Badge>
                      )}
                      {spot.metadata?.reservable === "true" && (
                        <Badge variant="outline" className="text-emerald-400 border-emerald-500/30 text-xs" data-testid={`badge-reservable-${spot.id}`}>
                  ...
                  {spot.metadata?.hookups && (
                    <div data-testid={`amenities-${spot.id}`}>
                      <Badge variant="secondary" className="text-xs" data-testid={`badge-hookups-${spot.id}`}>
                        {spot.metadata.hookups}
                      </Badge>
                    </div>
                  )}
        ...
        {filteredSpots.length === 0 && !isLoading && (
          ...
          >
            <TreePine className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-foreground mb-1" data-testid="text-no-results">No campgrounds found</h3>
